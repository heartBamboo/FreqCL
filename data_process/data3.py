...
def process_ToN_IoT(dataset_dir: str, output_dir: str, num_flow_packets: int = 5, header_length: int = 160, payload_length: int = 104, batch_size: int = 100, five_tuple_output_dir:str = None):
    tmp_output_dir = './data/tmp'
    flow_list = []

    benign_data_dir = f'{dataset_dir}/normal_pcaps'
    benign_pcaps = find_files(benign_data_dir)
    flow_list = process_unlabeled_pcap(benign_pcaps, output_dir, tmp_output_dir, num_flow_packets, header_length, payload_length, f'Process Benign',
                                       batch_size)
    # TODO other classes

    save_data(output_dir, flow_list, None)

def process_Bot_IoT(dataset_dir: str, output_dir: str, num_flow_packets: int = 5, header_length: int = 160, payload_length: int = 104, batch_size: int = 100):
    label_names = ['Benign',
                   # 'DoS',
                   # 'DDoS',
                   'Scan', 'Theft'
    ...
    for class_name in tqdm.tqdm(label_names[1:], desc='Process Bot-IoT'):
        ...
        for subclass_name in subclass_names:
            process_unlabeled_pcap(find_files(f'{dataset_dir}/{class_name}/{subclass_name}'), f'{output_dir}/{class_name}/{subclass_name}',
                                   tmp_output_dir, num_flow_packets, header_length, payload_length, f'Process {class_name}', batch_size)
...
def process_unlabeled_pcap(pcaps, output_dir: str, tmp_output_dir: str, num_flow_packets: int, header_length: int, payload_length: int, desc: str,
                           batch_size: int):
        ...
        for matched_pcap_path in tqdm.tqdm(batch_pcaps, total=len(batch_pcaps), desc=desc):
            ...
            for pcap_file in pcap_files:
                if process_ip(pcap_file, origin_pcap_file_name) is None:
                    continue
                try:
                    flow_data, directions, five_tuple = process_pcap_to_array(pcap_file, num_flow_packets, header_length, payload_length)
                except Scapy_Exception:
                    continue
...
def process_pretrain_dataset(output_dir: str, num_flow_packets:int = 5, header_length: int = 160, payload_length: int = 104):
    ...
    for dataset in ['ISCX-2012', 'CIC-IDS-2017']:
        ...
        flow_list = process_unlabeled_pcap(matched_pcaps, output_dir, tmp_output_dir, num_flow_packets, header_length, payload_length, f'Process {dataset}')
