...
class ToN_IoT_Train(Dataset):
    ...
    def __init__(self, root, train=True, transform=None,
        ...
        data_np, label_list, class_list = self.load_data('ToN_IoT', 128, train=self.train, train_ratio=0.8, seed=3407)
...
class ToN_IoT_Test(ToN_IoT_Train):
    """Base ToN_IoT dataset."""
    ...
    def __getitem__(self, index: int, ret_segmask=False) -> Tuple[type(Image), int, type(Image)]:
        ...
        if ret_segmask or self._return_segmask:
            ...
            seg = transforms.ToTensor()(transforms.CenterCrop((ToN_IoT_Train.IMG_SIZE, ToN_IoT_Train.IMG_SIZE))(seg))[0]
...
class Sequential_ToN_IOT(ContinualDataset):
    ...
    N_TASKS = 5
    SIZE = (ToN_IoT_Train.IMG_SIZE, ToN_IoT_Train.IMG_SIZE)
    # MEAN, STD = (0.4856, 0.4994, 0.4324), (0.2272, 0.2226, 0.2613) # RGB
    MEAN, STD = (0.4856,), (0.2272,)  # 单通道的均值和标准差
    TRANSFORM = transforms.Compose([
        #transforms.RandomCrop(ToN_IoT_Train.IMG_SIZE, padding=4),
        #transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(MEAN, STD)
        ])
    TEST_TRANSFORM = ToN_IoT_Train.TEST_TRANSFORM
    ...
    def get_data_loaders(self) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
        ...
        ToN_IoT_Train_dataset = ToN_IoT_Train(base_path() + 'ToN_IoT', train=True, transform=transform)
        ToN_IoT_Test_dataset = ToN_IoT_Test(base_path() + 'ToN_IoT', train=False, transform=test_transform)

        train, test = store_masked_loaders(
            ToN_IoT_Train_dataset, ToN_IoT_Test_dataset, self)
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), Sequential_ToN_IOT.TRANSFORM])
        return transform

    @staticmethod
    def get_DWT_transform():
        test_transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize(Sequential_ToN_IOT.MEAN, Sequential_ToN_IOT.STD)])
    ...
    @staticmethod
    def get_backbone():
        #return resnet18(Sequential_ToN_IOT.N_CLASSES_PER_TASK * Sequential_ToN_IOT.N_TASKS)
        num_classes = Sequential_ToN_IOT.N_CLASSES_PER_TASK * Sequential_ToN_IOT.N_TASKS
    ...
    def get_DWT_backbone():
        return DWT_resnet18(Sequential_ToN_IOT.N_CLASSES_PER_TASK * Sequential_ToN_IOT.N_TASKS)
        # num_classes = Sequential_ToN_IOT.N_CLASSES_PER_TASK * Sequential_ToN_IOT.N_TASKS
        # #MambaVision_B
    ...
    def get_normalization_transform():
        transform = transforms.Normalize(
            Sequential_ToN_IOT.MEAN, Sequential_ToN_IOT.STD)
        return transform

    @staticmethod
    def get_denormalization_transform():
        transform = DeNormalize(Sequential_ToN_IOT.MEAN, Sequential_ToN_IOT.STD)
        return transform
