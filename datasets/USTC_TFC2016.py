...
class USTC_TFC2016_Train(Dataset):
    """
    ...
    def __init__(self, root, train=True, transform=None,
        ...
        data_np, label_list, class_list = self.load_data('USTC_TFC2016', 128, train=self.train, train_ratio=0.8, seed=3407)
    ...
    def load_data(self, dataset_name: str, packet_length: int, train: bool = True, train_ratio: float = 0.8,
        ...
        label_map = torch.load(f'{dataset_dir}/label_map.pt')  # 注意：这是 {index: class_name}
        ...
        #     class_name = label_map.get(int(lbl), f"Class_{lbl}")
        #     print(f"  {class_name} (label={lbl}): {cnt} samples")
        #
        # # classes = [label_map[i] for i in sorted(label_map.keys())]  # 可选：按标签顺序返回类名
        # classes = list(label_map.values())  # 保持原始顺序
        # ===================统计并打印每个类别的样本数量===========================
...
class USTC_TFC2016_Test(USTC_TFC2016_Train):
    """Base USTC_TFC2016 dataset."""
    ...
    def __getitem__(self, index: int, ret_segmask=False) -> Tuple[type(Image), int, type(Image)]:
        ...
        if ret_segmask or self._return_segmask:
            ...
            seg = transforms.ToTensor()(transforms.CenterCrop((USTC_TFC2016_Train.IMG_SIZE, USTC_TFC2016_Train.IMG_SIZE))(seg))[0]
            ret_tuple.append((seg > 0).int())

        return ret_tuple


class Sequential_USTC_TFC2016(ContinualDataset):
    ...
    N_TASKS = 5
    SIZE = (USTC_TFC2016_Train.IMG_SIZE, USTC_TFC2016_Train.IMG_SIZE)
    # MEAN, STD = (0.4856, 0.4994, 0.4324), (0.2272, 0.2226, 0.2613) # RGB
    MEAN, STD = (0.4856,), (0.2272,)  # 单通道的均值和标准差
    TRANSFORM = transforms.Compose([
        #transforms.RandomCrop(USTC_TFC2016_Train.IMG_SIZE, padding=4),
        #transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(MEAN, STD)
        ])
    TEST_TRANSFORM = USTC_TFC2016_Train.TEST_TRANSFORM
    ...
    def get_data_loaders(self) -> Tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
        ...
        USTC_TFC2016_Train_dataset = USTC_TFC2016_Train(base_path() + 'USTC_TFC2016', train=True, transform=transform)
        USTC_TFC2016_Test_dataset = USTC_TFC2016_Test(base_path() + 'USTC_TFC2016', train=False, transform=test_transform)

        train, test = store_masked_loaders(
            USTC_TFC2016_Train_dataset, USTC_TFC2016_Test_dataset, self)
    ...
    def get_transform():
        transform = transforms.Compose(
            [transforms.ToPILImage(), Sequential_USTC_TFC2016.TRANSFORM])
        return transform

    @staticmethod
    def get_DWT_transform():
        test_transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize(Sequential_USTC_TFC2016.MEAN, Sequential_USTC_TFC2016.STD)])
    ...
    @staticmethod
    def get_backbone():
        #return resnet18(Sequential_USTC_TFC2016.N_CLASSES_PER_TASK * Sequential_USTC_TFC2016.N_TASKS)
        num_classes = Sequential_USTC_TFC2016.N_CLASSES_PER_TASK * Sequential_USTC_TFC2016.N_TASKS
    ...
    def get_DWT_backbone():
        #return DWT_resnet18(Sequential_USTC_TFC2016.N_CLASSES_PER_TASK * Sequential_USTC_TFC2016.N_TASKS)
        num_classes = Sequential_USTC_TFC2016.N_CLASSES_PER_TASK * Sequential_USTC_TFC2016.N_TASKS
        #MambaVision_B
    ...
    def get_normalization_transform():
        transform = transforms.Normalize(
            Sequential_USTC_TFC2016.MEAN, Sequential_USTC_TFC2016.STD)
        return transform

    @staticmethod
    def get_denormalization_transform():
        transform = DeNormalize(Sequential_USTC_TFC2016.MEAN, Sequential_USTC_TFC2016.STD)
        return transform
